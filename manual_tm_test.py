...
AND write results to the DB, without waiting for the hourly scheduler.

This mirrors scheduler._run_ticketmaster() exactly.
...
for city in cities:
    ...
    for ev in events:
        if insert_event(ev, city):
            ...
            skipped += 1
    print(f"=== {city['name']}: {len(events)} pulled, {inserted} inserted, {skipped} skipped ===\n")
