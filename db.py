...
def make_fingerprint(event: dict) -> str:
    # Use the UTC instant as the canonical time so the fingerprint is stable
    # regardless of local/UTC field labeling. Falls back to start_date, which
    # in old data held the same UTC clock value — keeping dedup consistent.
    canonical_start = event.get("start_utc") or event.get("start_date") or ""
    raw = "|".join([
        (event.get("title") or "").strip().lower(),
        canonical_start,
        (event.get("venue_name") or "").strip().lower(),
    ])
