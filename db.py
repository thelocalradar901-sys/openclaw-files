...
def make_fingerprint(event: dict, resolved_times: dict | None = None) -> str:
    ...
    raw = "|".join([
        normalize_title_for_matching(event.get("title") or ""),
        canonical_date,
...
# Known promotional/reseller suffix patterns that get appended to an
# otherwise-identical title by certain sources (confirmed: a BJCC-
# affiliated reseller source appends "| Official BJCC Ticket + Hotel
# Packages" or similar to nearly every listing). These suffixes are
# the reason "WWE Monday Night RAW" and "WWE Monday Night RAW |
# Official BJCC Ticket + Hotel Packages" only scored 0.52 similarity
# under plain difflib -- well below the 0.80 threshold -- and created
# a second post instead of being recognized as the same event.
#
# Matched case-insensitively, suffix only (re.IGNORECASE + $ anchor),
# so this never touches a title that legitimately contains "tour" or
# "official" as part of its real name elsewhere in the string.
...
def find_cross_source_match(conn, event: dict, resolved_times: dict) -> int | None:
    ...
    DIFFERENT source, where the title string doesn't match exactly --
    so the normal exact-hash fingerprint lookup misses it.

    Why this exists: make_fingerprint() hashes the title as an exact
    string. Two sources describing the same event almost never agree
    on title formatting byte-for-byte -- e.g. Ticketmaster's "Birmingham
    Legion FC vs. Brooklyn FC" vs. a venue calendar's "Birmingham Legion
    FC vs Brooklyn FC" (just a missing period) hash to two completely
    different fingerprints and were creating two separate posts for the
    same match, confirmed 2026-06-26. No amount of retry-loop or
    isolation-level fixing helps here -- those bugs were about two
    workers racing for the SAME fingerprint; this is a different
    problem, two DIFFERENT fingerprints for what's actually one event.

    Matching rule (deliberately conservative, to avoid merging two
    genuinely different events that happen to share a date): a
    candidate must have
      - the same city_slug, AND
      - the same canonical (date-only) start date, AND
      - a title at least TITLE_SIMILARITY_THRESHOLD similar via
        difflib.SequenceMatcher, AFTER both titles have known
        promotional/reseller suffixes stripped by
        normalize_title_for_matching() (e.g. "WWE Monday Night RAW"
        vs. "WWE Monday Night RAW | Official BJCC Ticket + Hotel
        Packages" only scored 0.52 similarity with the raw suffix
        still attached -- well under the 0.80 threshold -- so this
        normalization step runs before the similarity check, not just
        in make_fingerprint()).
        0.80 = "same words, minor punctuation/formatting differences"
        -- NOT "two different recurring nights with generic names like
        'Open Mic Night' at different venues," which would need to
        also match on date to even be considered, and even then often
        won't hit 80% similarity unless the venue name is baked into
        the title for both sources.

    Only called when the exact fingerprint lookup in insert_event()
    already missed -- this is strictly a fallback, never the primary
    path, so it doesn't change behavior for the common case where a
    single source re-scrapes its own event and the title matches
    exactly every time.

    Returns the matching post_id, or None if no good match is found.
    """
    canonical_date = (
        resolved_times.get("start_utc") or
        resolved_times.get("start_local") or ""
    ...
    city_slug = (event.get("city_slug") or "").strip().lower()
    title     = normalize_title_for_matching(event.get("title") or "")
    if not title or not city_slug:
        return None

    with conn.cursor() as cur:
        cur.execute(
            f"SELECT p.ID, p.post_title "
            f"FROM {WP_PREFIX}posts p "
            f"JOIN {WP_PREFIX}postmeta pm_date ON pm_date.post_id = p.ID "
            f"  AND pm_date.meta_key = '_EventStartDate' "
            f"  AND pm_date.meta_value LIKE %s "
            f"JOIN {WP_PREFIX}postmeta pm_city ON pm_city.post_id = p.ID "
            f"  AND pm_city.meta_key = '_openclaw_city' "
            f"  AND pm_city.meta_value = %s "
            f"WHERE p.post_type = 'tribe_events' "
            f"  AND p.post_status IN ('publish', 'draft')",
            (f"{canonical_date}%", city_slug)
        )
    ...
    if best_id:
        log.info(
            "Cross-source match: '%s' matched existing post %d '%s' (similarity %.2f)",
            event.get("title"), best_id,
            next(c["post_title"] for c in candidates if c["ID"] == best_id),
            best_ratio
        )
    return best_id
...
def insert_event(event: dict, city_config: dict = None) -> bool:
    ...
    # Cancelled events: don't create them, and if a previously-published
    # post for this same event already exists, delete it outright rather
    # than just updating its title to show "*Cancelled*". A cancelled
    # show has zero value to a visitor browsing "what's happening" --
    # confirmed by checking the live site, where a cancelled event was
    # showing a working "Buy Tickets" link with no real indication
    # (beyond asterisks in the title) that the show isn't happening.
    raw_title = (event.get("title") or "").strip()
    if raw_title and raw_title != _PROMO_SUFFIX_RE.sub("", raw_title).strip():
        # The title contains a stripped pattern -- but only the
        # cancellation patterns should trigger deletion, not the
        # "| Official ... Ticket + Hotel Packages" promo suffix or
        # "(SOLD OUT)", which are not cancellations and should still
        # be created/updated normally (just matched fuzzily). Check
        # specifically for the cancellation patterns here.
        is_cancelled = bool(re.search(
        ...
        if is_cancelled:
            conn = get_connection()
            try:
                existing_id = get_fingerprint_post_id(conn, fp) or find_cross_source_match(conn, event, t)
                if existing_id:
                    with conn.cursor() as cur:
                        cur.execute(f"DELETE FROM {WP_PREFIX}postmeta WHERE post_id=%s", (existing_id,))
                        cur.execute(f"DELETE FROM {WP_PREFIX}tec_events WHERE post_id=%s", (existing_id,))
                        cur.execute(f"DELETE FROM {WP_PREFIX}tec_occurrences WHERE post_id=%s", (existing_id,))
                        cur.execute(f"DELETE FROM {WP_PREFIX}term_relationships WHERE object_id=%s", (existing_id,))
                        cur.execute(f"DELETE FROM {WP_PREFIX}openclaw_fingerprints WHERE post_id=%s", (existing_id,))
                        cur.execute(f"DELETE FROM {WP_PREFIX}posts WHERE ID=%s", (existing_id,))
                    conn.commit()
                    log.info("Deleted cancelled event [%d] '%s'", existing_id, raw_title)
                else:
                    log.info("Skipped cancelled event (never existed) '%s'", raw_title)
            finally:
                conn.close()
            return False
