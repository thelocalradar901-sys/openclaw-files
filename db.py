...
def make_fingerprint(event: dict) -> str:
    ...
    Why: long-running exhibits/touring shows (e.g. "Bodies -- The
    Science Within") sell each daily timeslot as its own Ticketmaster
    event, and some calendar plugins (e.g. MEC on Choose901, for
    SciPlay-style recurring listings) render one calendar tile per day
    an ongoing event spans. Both produce many same-title rows on the
    same day that differ only by time-of-day or by which day's tile
    rendered them. Keeping time-of-day in the fingerprint made every
    one of those a "new" event instead of an update -- this is what
    caused the Bodies/Denver and SciPlay/Choose901 duplicate floods.
...
def insert_event(event: dict, city_config: dict = None) -> bool:
    ...
    try:
        ...
        try:
            ...
            conn.rollback()
            winner_id = get_fingerprint_post_id(conn, fp)
            if winner_id and winner_id != 0:
                return update_event(conn, winner_id, event, city_config)
            # Winner's INSERT committed the fp claim but may not have
            # finished writing its post_id yet (placeholder still 0).
            # Brief wait-and-retry rather than risk a duplicate post.
            for _ in range(10):
                time.sleep(0.2)
                winner_id = get_fingerprint_post_id(conn, fp)
                if winner_id and winner_id != 0:
        ...
        with conn.cursor() as cur:
            ...
            # Fill in the real post_id on our fingerprint claim row
            # (was reserved as 0 above).
            cur.execute(
