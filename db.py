...
def find_cross_source_match(conn, event: dict, resolved_times: dict) -> int | None:
    ...
    candidate must have
      ...
      - a title at least TITLE_SIMILARITY_THRESHOLD similar via
        difflib.SequenceMatcher (0.80 = "same words, minor punctuation/
        formatting differences" -- NOT "two different recurring nights
        with generic names like 'Open Mic Night' at different venues,"
        which would need to also match on date to even be considered,
        and even then often won't hit 80% similarity unless the venue
        name is baked into the title for both sources).
    ...
    best_ratio = 0.0
    for c in candidates:
        candidate_title = (c["post_title"] or "").strip().lower()
