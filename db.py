...
def insert_event(event: dict, city_config: dict = None) -> bool:
    ...
    try:
        ...
        try:
            ...
            # Widened from 10x0.2s (2s) to 25x0.2s (5s): the previous
            # window was shorter than the winner's own image-sideload +
            # category + TEC-index writes could take, so the loser
            # routinely gave up and logged "race unresolved" while the
            # winner's row was still mid-write rather than missing.
            # This fix also makes that wait far less likely to matter,
            # since the post_id backfill below now happens immediately
            # after the post row is created instead of after those
            # slower writes.
            for _ in range(25):
                time.sleep(0.2)
                winner_id = get_fingerprint_post_id(conn, fp)
