...
def scrape_squarespace(source, city):
    ...
    for ical_url in ical_links:
        ...
        for component in cal.walk():
            ...
            # Fetch full detail page for image + rich description
            detail_image, detail_desc = _squarespace_detail(event_url) if event_url else ("", "")
            image_url = detail_image
            if detail_desc and len(detail_desc) > len(desc):
                desc = detail_desc
...
def _squarespace_detail(event_url):
    """Fetch Squarespace event detail page and return (image_url, description)."""
    if not event_url:
        return "", ""
    r = _get(event_url)
    if not r:
    ...
    # Description: og:description or main event body text
    desc = ""
    og_desc = soup.find("meta", property="og:description") or soup.find("meta", attrs={"name": "description"})
    if og_desc and og_desc.get("content"):
        desc = og_desc["content"].strip()
    if not desc:
        # Try event description block
        for sel in ["div.eventitem-column-content", "div.sqs-block-content", "div.event-excerpt"]:
            block = soup.select_one(sel)
            if block:
                desc = block.get_text(" ", strip=True)[:1000]
                break
