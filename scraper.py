...
def _fallback_heading_containers(soup) -> list:
    ...
    for tag in soup.find_all(["h1", "h2", "h3", "h4"]):
        a = tag.find("a", href=True) or (
            tag.find_next_sibling("a", href=True)
        )
        if not a:
            continue
    ...
    for tag, href in candidates:
        ...
        if not parts or parts[0] != top_seg:
            continue
        container = _synthetic_container(tag, soup)
...
def _synthetic_container(heading_tag, soup):
    """
...
def _parse_heuristic(html: str, base_url: str, source: dict,
    ...
    for c in containers[:60]:
        ...
        if not date_str:
            ...
            # a date-shaped fragment (e.g. "Thu, Jun 18") and append any
            # nearby time text (e.g. "Doors: 6 pm // Show: 7 pm") so the
            # fuzzy parser has both a date and a time to work with.
            full_text = c.get_text(" ", strip=True)
            date_match = _LOOSE_DATE_RE.search(full_text)
            if date_match:
                date_str = date_match.group(0)
                time_match = _SHOW_TIME_RE.search(full_text)
                if time_match:
        ...
        for cand in c.find_all("a", href=True):
            ...
            if any(kw in link_text for kw in ("buy ticket", "free show", "rsvp", "ticket", "get ticket")):
                a_tag = cand
                break
        if not a_tag:
