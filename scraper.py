...
def _scrape_html_auto(source: dict, city_slug: str, city_name: str,
    ...
    # Tier 2: JSON-LD
    events = _parse_jsonld(html, source, city_slug, city_name, tz_name)
    if events:
...
def _parse_jsonld(html: str, source: dict, city_slug: str, city_name: str,
                  tz_name: str = "America/Chicago") -> list[dict]:
    import json
    soup   = BeautifulSoup(html, "html.parser")
    events = []
    for script in soup.find_all("script", {"type": "application/ld+json"}):
        ...
        for item in items:
            ...
            events.append(_ev(
                ...
                start_date=_normalize_dt(start_raw, tz_name),
                end_date=_normalize_dt(item.get("endDate", start_raw), tz_name),
                venue_name=venue_name,
...
def _parse_heuristic(html: str, base_url: str, source: dict,
    ...
    for c in containers[:60]:
        ...
        # Use _normalize_dt with tz_name so UTC datetimes get converted to local
...
def _normalize_dt(raw, tz_name: str = None) -> str:
    """Parse a date string. If UTC-aware and tz_name given, convert to local time."""
    if not raw:
        return ""
    ...
    raw = str(raw).strip()
    # Handle Z suffix
    has_utc = raw.endswith("Z") or "+00:00" in raw or "T" in raw and raw.endswith("Z")
    raw_iso  = raw.replace("Z", "+00:00")
    if re.match(r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$", raw):
        return raw
    try:
        dt = datetime.fromisoformat(raw_iso)
        # If timezone-aware and we have a target tz, convert
