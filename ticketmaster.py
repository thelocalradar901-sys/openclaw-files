...
def pull_city(city: dict) -> list[dict]:
    """Pull all upcoming TM events for a city. Returns list of normalized event dicts."""
    ...
    # Use city's IANA timezone for local time conversion
    # Fall back to Central since all current cities are Central
    ...
    end_str   = (now_utc + timedelta(days=90)).strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    while True:
        ...
        try:
            ...
            log.error("TM API error page %d for %s: %s", page, city["name"], e)
            break
        ...
        log.debug("TM %s page %d/%d — %d events", city["name"], page + 1, total_pages, len(events))
    ...
    log.info("TM pulled %d events for %s", len(events), city["name"])
    return events
